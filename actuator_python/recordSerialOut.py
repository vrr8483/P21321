...
ser = serial.Serial('/dev/tty.usbmodem14201', 38400)
dataSetCache = {}
...
class dataSet(object):
    ...
    def __init__(self, tag):
        ...
        if not self.initialized:
            ...
            # print("Opening ", str(tag), ".csv")
            self.data = []
            self.time = []
            # Default to plot the first tag you see
            if dataSet.plottingTag is None:
    ...
    def addDataPoint(self, line):
        ...
        # self.time.append(int(tokens[0]))
        # self.data.append(int(tokens[1]))
...
def serialListener():
    ...
    while True:
        ...
        try:
            ...
            # must begin with tag, exclamation point marks a command the other thread sent,
            # and disregard the first second
            # if (not listenerString[0].isalpha()) or ('q' in listenerString) or pointsCollected < 5:
            #     pass
            # # Did we already create this dataset?
            # el
            # if listenerString[0] in dataSetCache.keys():
            if data_created:
                # Yes it is created, go ahead and use the one we made previously
                data = dataSetCache.get(data_key)
                ...
                data.addDataPoint(listenerString)
                dataSetCache[data_key] = data  # 100% forgot this line, this might break things
            # Reset string
            pointsCollected += 1
